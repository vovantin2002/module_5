...
function List() {
    ...
    const getAll = async () => {
        const result = await axios.get('http://localhost:8080/houseService');
    ...
    const deleteCustomer = async (id) => {
        await axios.delete(`http://localhost:8080/houseService/${id}`)
        await alert("Xoa thanh cong")
        getAll();
    }
    return (
        <>
            <h1><Link to={"/house/create"}>Create</Link></h1>
            <div className="row">
